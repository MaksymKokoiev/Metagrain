...
namespace Metasound
{
            ...
            auto CreateNodeClassMetadata = []() -> FNodeClassMetadata
                {
                    ...
                    Metadata.Author = TEXT("Maksym Kokoiev");
    ...
    private:
        ...
        bool InitializeWaveData(const FSoundWaveProxyPtr& InSoundWaveProxy)
        {
            ...
            else {UE_LOG(LogMetaSound, Error, TEXT("GS: Failed to set multichannel buffer (Channels: %d, Frames: %d)"),CurrentNumChannels, DeinterleaveBlockSizeFrames);
                return false; }
        }
        ...
        {
            if (CachedSoundWaveDuration > 0.0f)
            {
                float GrainDurationSec = static_cast<float>(InGrainDurationSamples) / SampleRate;
                float RequiredSourceDuration = GrainDurationSec * InFrameRatio;
                float ValidRegionLength = CachedSoundWaveDuration - RequiredSourceDuration;
                if (ValidRegionLength > 0.0f)
                {
                    InStartTimeSeconds = FMath::Fmod(InStartTimeSeconds, ValidRegionLength);
                    if (InStartTimeSeconds < 0.0f)
                    {
                        InStartTimeSeconds += ValidRegionLength;
                    }
                }
                else
                {
                    InStartTimeSeconds = 0.0f;
                }
            }
